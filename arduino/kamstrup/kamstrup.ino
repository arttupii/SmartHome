...
word const kregnums[] = { 
  0x003C,0x0050,0x0056,0x0057,0x0059,0x004a,0x0044 };                                                   // The registers we want to get out of the meter
char* kregstrings[]   = { 
  "Energy","Current Power","Temperature t1","Temperature t2","Temperature diff", "Flow", "Volumen 1" }; // The name of the registers we want to get out of the meter in the same order as above
#define NUMREGS 7                                                                                                               // Number of registers above
#define KAMBAUD 1200                                                                                                            // The meter's IR runs only at 1200 BAUD
...
//********************************************Kamstrup Stuff Start********************************************
// Pin definitions
#define PIN_KAMSER_RX  6                                                                                                        // Kamstrup IR interface RX --- CORRECT THIS ONE <<<<<<<<<<<<<<<<<<<<<<<<<<
...
void setup() {
  Serial.begin(57600);
  Serial.println("emonTX Kamstrup Multical + Temp + Pulse example"); 
