...
void Json::add(const char* name, int val){
	addDefault(name);
	Serial.print(val);
}
...
void Json::add(const char* name, const char *val){
	addDefault(name);
	Serial.print("\"");
	Serial.print(val);
	Serial.print("\"");
...
void Json::addArray(const char* name, const unsigned char *val, int len){
	addDefault(name);
	Serial.print("[");
	for(int i=0;i<len;i++){
		if(i!=0) Serial.print(",");
...
void Json::addHexStr(const char* name, const unsigned char *val, int len){
	addDefault(name);
	Serial.print("\"");
	for(int i=0;i<len;i++){
		char buf[10];
