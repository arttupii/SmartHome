...
def License():
    temps = string.split(LICENSE, '\n')
    for temp in temps:
        if len(temp) > 0 and temp[0] == '#':
            temp = temp[1:]
        if len(temp) > 0 and temp[0] == ' ':
            temp = temp[1:]
        print string.strip(temp)
...
def GetDials(conf):
    ...
    for cd in confdials:
        ...
        if len(cdl) == 8:
            try:
                cx = int(string.strip(cdl[0]))
                cy = int(string.strip(cdl[1]))
                mrad = float(string.strip(cdl[2]))
                irad = float(string.strip(cdl[3]))
                dangle = float(string.strip(cdl[4]))
                dangle2 = float(string.strip(cdl[5]))
                za = float(string.strip(cdl[6]))
                rd = int(string.strip(cdl[7]))
...
def GetPosareas(conf):
    ...
    for cd in confposareas:
        ...
        if len(cdl) == 4:
            try:
                x1 = int(string.strip(cdl[0]))
                y1 = int(string.strip(cdl[1]))
                x2 = int(string.strip(cdl[2]))
                y2 = int(string.strip(cdl[3]))
...
def GetShakes(conf):
    ...
    for cs in confshakes:
        ...
        if len(csl) == 3:
            try:
                dx = int(string.strip(csl[0]))
                dy = int(string.strip(csl[1]))
                rangle = float(string.strip(csl[2]))
...
def CommandMeter(conf, meterfile):     
    ...
    filtername = string.upper(conf.getValue('IMAGE_FILTER', 'NONE'))
    ...
    finstr = CalcCounterValue(values)
